...
const useRecentsStore = create<RecentsStore>()(
  persist(
    (set) => ({
      recents: [],
      ...
      addRecent: (event) =>
        set((state) => ({
          recents: [
            { ...state.recents.find((e) => e.id === event.id), ...event },
            ...state.recents.filter((e) => e.id !== event.id),
          ],
        })),
      removeRecent: (id) =>
        set((state) => ({
      ...
      migrate: (persistedState, version) => {
        if (version === 0) {
          return {
            ...
            recents: (
              persistedState as {
                recents: { id: string; name: string; created: number }[];
              }
            ).recents.map((ev) => ({
              id: ev.id,
              name: ev.name,
