...
export const getStats = () =>
  get("/stats", StatsResponse, undefined, { revalidate: 60 });
export const getEvent = (eventId: string) =>
  get(`/event/${eventId}`, EventResponse);
export const getPeople = (eventId: string) =>
  get(`/event/${eventId}/people`, PersonResponse.array());
...
export const createEvent = (input: EventInput) =>
  post("/event", EventResponse, EventInput.parse(input));
