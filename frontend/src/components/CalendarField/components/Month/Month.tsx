...
const Month = ({ value, onChange }: MonthProps) => {
  ...
  const [page, setPage] = useState<Temporal.PlainYearMonth>(
    now.toPlainYearMonth()
  );
  const dates = useMemo(
    () => calculateMonth(page, weekStart, i18n.language),
    [page, weekStart, i18n.language]
  );
  ...
  const hasPastDates = useMemo(
    () =>
      value.some(
        (plainDate) =>
          Temporal.PlainDate.compare(now, Temporal.PlainDate.from(plainDate)) >
          0
      ),
    [value]
  );
      ...
      {useMemo(
        () => (
          <div className={styles.dayLabels}>
            {rotateArray(
              getWeekdayNames(i18n.language, "short"),
              weekStart ? 0 : 1
            ).map((name) => (
            ...
            dateRow.map((date, x) => (
              <button
                ...
                className={makeClass(
                  ...
                  ((!(
                    mode.current === "remove" && selecting.includes(date.string)
                  ) &&
                    value.includes(date.string)) ||
                    (mode.current === "add" &&
                      selecting.includes(date.string))) &&
                    styles.selected
                )}
                key={date.string}
                title={`${date.title}${
                  date.isToday ? ` (${t("form.dates.tooltips.today")})` : ""
                }`}
                onKeyDown={(e) => {
...
const calculateMonth = (
  ...
  const daysAfter =
    6 - month.toPlainDate({ day: month.daysInMonth }).dayOfWeek + weekStart;
