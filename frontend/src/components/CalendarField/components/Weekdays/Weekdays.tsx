      ...
      rotateArray(
        range(1, 7).map((i) =>
          Temporal.Now.plainDateISO().add({
            days: i - Temporal.Now.plainDateISO().dayOfWeek,
          })
        ),
        weekStart ? 0 : 1
      ),
      ...
      {weekdays.map((day, i) => (
        <button
          ...
          className={makeClass(
            ...
            ((!(
              mode.current === "remove" &&
              selecting.includes(day.dayOfWeek.toString())
            ) &&
              value.includes(day.dayOfWeek.toString())) ||
              (mode.current === "add" &&
                selecting.includes(day.dayOfWeek.toString()))) &&
              styles.selected
          )}
          key={day.toString()}
          title={
            day.equals(Temporal.Now.plainDateISO())
              ? t("form.dates.tooltips.today")
              : undefined
          }
          onKeyDown={(e) => {
          ...
          onPointerDown={(e) => {
            startPos.current = i;
            mode.current = value.includes(day.dayOfWeek.toString())
              ? "remove"
              : "add";
            setSelecting([day.dayOfWeek.toString()]);
            e.currentTarget.releasePointerCapture(e.pointerId);

            document.addEventListener("pointerup", handleFinishSelection, {
              once: true,
            });
          }}
          onPointerEnter={() => {
            if (mode.current) {
              const found = [];
              for (
                let ci = Math.min(startPos.current, i);
                ci < Math.max(startPos.current, i) + 1;
                ci++
              ) {
