...
const CreateForm = ({ noRedirect }: { noRedirect?: boolean }) => {
  ...
  const onSubmit: SubmitHandler<Fields> = async (values) => {
    ...
    try {
        ...
        return hours.map((hour) => {
          ...
          if (isSpecificDates) {
            ...
            return `${dateTime.hour
              ...
              .toString()
              .padStart(2, "0")}${dateTime.month
              .toString()
              .padStart(2, "0")}${dateTime.year.toString().padStart(4, "0")}`;
          } else {
            // Format as `HHmm-d`
            return `${dateTime.hour
              .toString()
              ...
              .toString()
              .padStart(2, "0")}-${String(
              dateTime.dayOfWeek === 7 ? 0 : dateTime.dayOfWeek
            )}`;
          }
        });
      });
      ...
      const newEvent = await createEvent({ name, times, timezone }).catch(
        (e) => {
          console.error(e);
