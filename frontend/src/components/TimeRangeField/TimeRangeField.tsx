...
interface TimeRangeFieldProps<TValues extends FieldValues>
...
const Selection = ({ start, end }: { start: number; end: number }) => (
  <div
...
const Handle = ({ value, onChange, labelPadding }: HandleProps) => {
  ...
  const handleMouseMove = (e: MouseEvent) => {
    if (isMoving.current) {
      let step = Math.round(
        ((e.pageX - rangeRect.current.left) / rangeRect.current.width) * 24
      );
      if (step < 0) step = 0;
      if (step > 24) step = 24;
      step = Math.abs(step);
  ...
  return (
    <div
      ...
      data-label={Temporal.PlainTime.from({
        hour: Number(times[value] === "24" ? "00" : times[value]),
      ...
      onTouchMove={(e) => {
        ...
        let step = Math.round(
          ((touch.pageX - rangeRect.current.left) / rangeRect.current.width) *
            24
        );
        if (step < 0) step = 0;
        if (step > 24) step = 24;
