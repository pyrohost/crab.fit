...
const AvailabilityEditor = ({
  ...
  const mode = useRef<"add" | "remove" | undefined>(undefined);
        ...
        <Content>
          ...
          >
            <GoogleCalendar
              ...
              timeEnd={parseSpecificDate(times[times.length - 1]).add({
                minutes: 15,
              })}
              times={times}
              onImport={onChange}
            />
            <RecentEvents eventId={eventId} times={times} onImport={onChange} />
          </div>
        </Content>
      )}
        ...
        <div>
          <div className={viewerStyles.heatmap}>
            <div className={viewerStyles.timeLabels}>
              {table?.rows.map((row, i) => (
                <div className={viewerStyles.timeSpace} key={i}>
                  {row && (
                    <label className={viewerStyles.timeLabel}>
                      {row.label}
                    </label>
                  )}
                </div>
            ...
            {table?.columns.map((column, x) => (
              <Fragment key={x}>
                {column ? (
                  <div className={viewerStyles.dateColumn}>
                    {column.header.dateLabel && (
                      <label className={viewerStyles.dateLabel}>
                        {column.header.dateLabel}
                      </label>
                    )}
                    <label className={viewerStyles.dayLabel}>
                    ...
                    >
                      {column.cells.map((cell, y) => {
                        ...
                        const isSelected =
                          (!(
                            mode.current === "remove" &&
                            selecting.includes(cell.serialized)
                          ) &&
                            value.includes(cell.serialized)) ||
                          (mode.current === "add" &&
                            selecting.includes(cell.serialized));
                        ...
                        return (
                          <div
                            ...
                            className={makeClass(
                              viewerStyles.time,
                              selecting.length === 0 && viewerStyles.editable
                            )}
                            style={
                              {
                                touchAction: "none",
                            ...
                            onPointerDown={(e) => {
                              ...
                              mode.current = value.includes(cell.serialized)
                                ? "remove"
                                : "add";
                            ...
                            onPointerEnter={() => {
                              if (mode.current) {
                                const found = [];
                                for (
                                  let cy = Math.min(startPos.current.y, y);
                                  cy < Math.max(startPos.current.y, y) + 1;
                                  cy++
                                ) {
                                  for (
                                    let cx = Math.min(startPos.current.x, x);
                                    cx < Math.max(startPos.current.x, x) + 1;
                                    cx++
                                  ) {
                  ...
                  <div className={viewerStyles.columnSpacer} />
                )}
              </Fragment>
            )) ?? <Skeleton isSpecificDates={times[0]?.length === 13} />}
          </div>
        </div>
      </div>
    </>
