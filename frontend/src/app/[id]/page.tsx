...
export const generateMetadata = async ({
  ...
  const event = await getEvent(_params.id).catch(() => undefined);
          ...
          >
            {t("common:created", {
              date: relativeTimeFormat(
                Temporal.Instant.fromEpochSeconds(event.created_at),
                i18n.language
              ),
            })}
          </span>
            ...
            <Trans i18nKey="event:nav.shareinfo" t={t} i18n={i18n}>
              ...
              <a
                href={`mailto:?subject=${encodeURIComponent(
                  t("event:nav.email_subject", { event_name: event.name })
                )}&body=${encodeURIComponent(
                  `${t("event:nav.email_body")} https://crab.fit/${event.id}`
                )}`}
              >
                _
              </a>
